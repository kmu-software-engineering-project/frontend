...
1. `CLAUDE.md` 읽기 — 4단계 품질 게이트, 보안 규칙
2. `docs/sprint/sprint-tracker.md` — 현재 스프린트 QA 대상 범위 확인
3. `docs/project-structure.md` — 실제 파일 경로 확인 (에이전트 문서와 다를 수 있음)
4. 이 파일의 검증 우선순위 및 출력 포맷 재확인

## 추가 검증 항목 (보안·환경)

아래 항목을 기존 검증 체크리스트에 포함한다:

| 항목 | 검증 방법 |
|------|----------|
| API 키 하드코딩 여부 | `process.env.` 없이 키 패턴 문자열이 코드에 존재하는지 Grep |
| `NEXT_PUBLIC_` 접두사 규칙 | 서버 전용 키에 `NEXT_PUBLIC_` 붙어있는지 확인 |
| `.env` 커밋 여부 | `.gitignore`에 `.env`, `.env.local` 포함 확인 |
| `'use client'` 누락 | 클라이언트 훅(useState, useEffect 등) 사용 파일에 선언 여부 |
| `console.log` 잔류 | 릴리즈 전 제거 대상 확인 |
